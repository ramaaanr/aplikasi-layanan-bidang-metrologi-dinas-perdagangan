  ...
  <div id="alert-additional-content-1" class="p-4 mb-4 text-dark-grey border rounded-lg " role="alert">
    ...
    <ul class="grid lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 gap-3">
      ...
      <li class="w-full border border-light-grey rounded-md hover:bg-light-2">
        <a href="/layanan/data-tera/{{$teraKey}}" class="relative w-full flex items-center justify-center px-5 py-2.5 text-sm font-medium text-center text-dark-grey hover:text-dark  rounded-lg  focus:ring-4 ">
          <span class="sr-only">Notifications</span>
          {{$this->formatString($teraKey)}}
          @if ($teraValue == 0)
          <div class="absolute inline-flex items-center justify-center w-6 h-6 text-xs font-bold text-white bg-green-500 border-2 border-white rounded-full -top-2 -right-2 dark:border-gray-900">
          </div>
          @else
          ...
          <div class="animate-ping absolute inline-flex items-center justify-center w-6 h-6  bg-red-300 border-2 border-white rounded-full -top-2 -right-2 dark:border-gray-900">
          </div>
          @endif
        </a>
      </li>
